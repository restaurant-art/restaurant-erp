...
(async () => {
  ...
  try {
    ...
    await globalAdmin.locator('nav').getByRole('button', { name: 'POS Billing', exact: true }).click();
    await globalAdmin.getByRole('button', { name: /QA Global Cashier/ }).click();
    await globalAdmin.getByPlaceholder('Enter cashier password').fill('test-only-password');
    await globalAdmin.getByRole('button', { name: /Continue to POS/ }).click();
    await globalAdmin.waitForFunction(() => localStorage.getItem('vestora-selected-store') === 'QA-A');
    await globalAdmin.context().close();
    console.log('PASS: all-stores user creation saves the chosen branch and POS lists its cashier');
